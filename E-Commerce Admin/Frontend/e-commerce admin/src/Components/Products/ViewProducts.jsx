...
function ViewProducts() {
  ...
  useEffect(() => {
    const fatchingdata = async () => {
      await axios
        .get('http://localhost:6099/Product/viewproduct')
        .then((res) => {
  ...
  const deleteProduct = async (id) => {
    try {
      const response = await axios.delete(`http://localhost:6099/Product/deleteproduct?id=${id}`);
      ...
      <div class="container-fluid page-body-wrapper">
        ...
        <div class="main-panel">
          ...
          <h3 class="page-title">
            ...
            <div className="m1" style={{width:"100%",display:"flex",justifyContent:"space-between"}}>

            {  data.length > 0 ? (
                data.map((el,i)=>{
                return  <div class="card" style={{ width: "20%" }}>
                  <img src={`http://localhost:6099/${el.image}`} class="card-img-top" alt="..." />
                  <div class="card-body">
                    <h3 class="card-title " style={{color : "white"}}>{el.prname}</h3>
                    <h5 class="card-title text-success" >{el.prprice} $ <br /> <br />
                    <span><span className="text-warning">Rating : {el.rating}</span> &nbsp;
                     </span>
                     </h5>
                    
                    <p class="card-text" ><span  style={{color : "gray"}}>{el.descripation}</span></p>
                    
                    
                    <a href="#" class="btn btn-danger" onClick={()=>deleteProduct(el._id)}>Delete</a>  
              ...
              <tbody>
                { data.length > 0 ? (
                  ...
                  data.map((el, i) => {
                    ...
                    return <tr key={i}>
                      <td>{i + 1}</td>
                      <td><img src={`http://localhost:6099/${el.image}`} alt="" style={{
                        width: "50px",
                        height: "50px",
                        objectFit: "cover",
                        borderRadius: "0", // Ensures no rounding
                      ...
                      <td>
                        ...
                        <button type="button" className="btn btn-danger m-1" onClick={() => deleteProduct(el._id)}>Delete</button>
                      </td>
                    </tr>
